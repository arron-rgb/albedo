...
public class PageableParameterBuilderPlugin implements OperationBuilderPlugin {
  ...
  /**
   * Constant <code>SORT_DESCRIPTION="Sorting criteria in the concatFilePath: propert"{trunked}</code>
   */
  public static final String SORT_DESCRIPTION =
    "Sorting criteria in the concatFilePath: property(,asc|desc). " + "Default sort order is ascending. ";
   ...
   *
   * @param nameExtractor
   *          a {@link springfox.documentation.schema.TypeNameExtractor} object.
   * @param resolver
   *          a {@link com.fasterxml.classmate.TypeResolver} object.
   */
   ...
   *
   * @param context
   *          {@link com.baomidou.mybatisplus.core.metadata.IPage} parameter context
  ...
  protected Parameter createSortParameter(ParameterContext context) {
    ...
    return new ParameterBuilder().name(getSortName()).parameterType(SORT_TYPE).modelRef(stringModel).allowMultiple(true)
   ...
   *
   * @param context
   *          a {@link springfox.documentation.spi.service.contexts.ParameterContext} object.
   * @return a {@link java.util.function.Function} object.
   */
  protected Function<ResolvedType, ? extends ModelReference> createModelRefFactory(ParameterContext context) {
    ModelContext modelContext = inputParam(context.getGroupName(), context.resolvedMethodParameter().getParameterType(),
