...
public class RoleResourceIntTest {
  ...
  public void getRolePage() throws Exception {
    ...
    // Get all the roles
    restRoleMockMvc
      .perform(get(DEFAULT_API_URL).param(PageModel.F_DESC, Role.F_SQL_CREATED_DATE).accept(MediaType.APPLICATION_JSON))
  ...
  public void getRole() throws Exception {
    ...
    restRoleMockMvc.perform(get(DEFAULT_API_URL + "{id}", roleDto.getId())).andExpect(status().isOk())
      ...
      .andExpect(jsonPath("$.data.name").value(DEFAULT_NAME)).andExpect(jsonPath("$.data.code").value(DEFAULT_CODE))
  ...
  public void updateRole() throws Exception {
    ...
    List<RoleMenu> listRoleMenuEntities =
      roleMenuService.list(Wrappers.<RoleMenu>query().lambda().eq(RoleMenu::getRoleId, testRole.getId()));
    ...
    List<RoleDept> listRoleDept =
      roleDeptService.list(Wrappers.<RoleDept>query().lambda().eq(RoleDept::getRoleId, testRole.getId()));
  ...
  public void deleteRole() throws Exception {
    ...
    // Delete the role
    restRoleMockMvc.perform(delete(DEFAULT_API_URL + "{id}", roleDto.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
