  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        <el-input v-model="query.title" class="filter-item input-small" clearable size="small" placeholder="输入标题搜索" @keyup.enter.native="toQuery" />
    ...
    >
      <el-form ref="form" :inline="true" :model="form" label-width="120px" size="small">
        <el-form-item label="标题" prop="title" :rules="[{min: 0,max: 32,message: '长度在 0 到 32 个字符', trigger: 'blur'},]">
          <el-input v-model="form.title" class="input-small" />
        ...
        <el-form-item label="作者" prop="author" :rules="[{required: true,message: '请输入作者', trigger: 'blur'},{min: 0,max: 50,message: '长度在 0 到 50 个字符', trigger: 'blur'},]">
          <el-input v-model="form.author" class="input-small" />
        ...
        <el-form-item label="邮箱" prop="email" :rules="[{min: 0,max: 100,message: '长度在 0 到 100 个字符', trigger: 'blur'},]">
          <el-input v-model="form.email" class="input-small" />

        </el-form-item>
        <el-form-item label="手机" prop="phone" :rules="[{min: 0,max: 32,message: '长度在 0 到 32 个字符', trigger: 'blur'},]">
          <el-input v-model="form.phone" class="input-small" />

        </el-form-item>
        <el-form-item label="activated" prop="activated" :rules="[{required: true,message: '请输入activated', trigger: 'blur'},{validator:validateDigits},]">
          <el-input v-model="form.activated" class="input-small" />

        </el-form-item>
        <el-form-item label="key" prop="number" :rules="[{validator:validateDigits},]">
          <el-input v-model="form.number" class="input-small" />

        </el-form-item>
        <el-form-item label="money" prop="money" :rules="[{ validator:validateNumber},]">
          <el-input v-model="form.money" class="input-small" />

        </el-form-item>
        <el-form-item label="amount" prop="amount" :rules="[{ validator:validateNumber},]">
          <el-input v-model="form.amount" class="input-small" />
        ...
        <el-form-item label="备注" prop="description" :rules="[{min: 0,max: 255,message: '长度在 0 到 255 个字符', trigger: 'blur'},]">
          <el-input v-model="form.description" class="input-small" />
    ...
    >
      ...
      <el-table-column align="center" label="activated" :show-overflow-tooltip="true" prop="activated" />
      <el-table-column align="center" label="key" :show-overflow-tooltip="true" prop="number" />
      <el-table-column align="center" label="money" :show-overflow-tooltip="true" prop="money" />
      <el-table-column align="center" label="amount" :show-overflow-tooltip="true" prop="amount" />
...
<script>
import crudTestBook from '@/views/biz/test-book/test-book-service'
...
export default {
  ...
  cruds() {
    return CRUD({ title: 'test', crudMethod: { ...crudTestBook }})
  },
  ...
  data() {
    ...
    validateDigits: (rule, value, callback) => {
      ...
      permission: {
        edit: 'biz_testBook_edit',
        del: 'biz_testBook_del'
      }
    }
  },
  computed: {
