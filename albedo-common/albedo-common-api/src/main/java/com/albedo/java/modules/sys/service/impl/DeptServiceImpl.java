...
public class DeptServiceImpl extends TreeServiceImpl<DeptRepository, Dept, DeptDto> implements DeptService {
  ...
  public boolean removeByIds(Set<String> ids) {
    ...
    ids.forEach(id -> {
      ...
      // 级联删除部门
      Set<String> idList =
        deptRelationService.list(Wrappers.<DeptRelation>query().lambda().eq(DeptRelation::getAncestor, id)).stream()
  ...
  public void lockOrUnLock(Set<String> ids) {
    repository.selectBatchIds(ids).forEach(dept -> {
      checkDept(dept.getId(), dept.getName());
      dept.setAvailable(CommonConstants.YES.equals(dept.getAvailable()) ? CommonConstants.NO : CommonConstants.YES);
  ...
  private void checkDept(String deptId, String deptName) {
    ...
    if (CollUtil.isNotEmpty(userList)) {
      throw new BadRequestException(
        "操作失败！用户：" + CollUtil.convertToString(userList, User.F_USERNAME, StringUtil.COMMA) + "所属要操作的部门：" + deptName);
    }
    List<Role> roleList = roleRepository.findListByDeptId(deptId);
    if (CollUtil.isNotEmpty(roleList)) {
      throw new BadRequestException(
        "操作失败！角色：" + CollUtil.convertToString(roleList, Role.F_NAME, StringUtil.COMMA) + "的权限信息属于要操作的部门：" + deptName);
    }
  }
  ...
  public List<String> findDescendantIdList(String deptId) {
    List<String> descendantIdList =
      deptRelationService.list(Wrappers.<DeptRelation>query().lambda().eq(DeptRelation::getAncestor, deptId)).stream()
