...
public class GenUtil {
  ...
  private static void initDataColumn(TableColumnDto column) {
    ...
    // 创建时间、更新时间
      ...
      || StringUtil.startWithIgnoreCase(column.getName(), BaseDataEntity.F_LASTMODIFIEDDATE)) {
          ...
          if (list != null) {
            for (String s : list) {
              if (StringUtil.startWith(s, GenCategory.CATEGORY_REF)) {
                templateList
                  .addAll(getTemplateList(config, StringUtil.replace(s, GenCategory.CATEGORY_REF, ""), false));
              } else {
  ...
  public static Map<String, Object> getDataModel(SchemeDto scheme) {
    ...
    model.put("permissionPrefix", model.get("moduleName")
      + (StringUtil.isNotBlank(scheme.getSubModuleName()) ? "_" + StringUtil.lowerCase(scheme.getSubModuleName()) : "")
  ...
  public static String generateToFile(TemplateVo tpl, Map<String, Object> model, boolean isReplaceFile) {
    ...
    String realFileName = FreeMarkers.renderString(tpl.getFileName(), model),
      fileName = StringUtil.getProjectPath(realFileName, getConfig().getCodeUiPath()) + File.separator + FreeMarkers
        .renderString(tpl.getFilePath() + StringUtil.SLASH, model).replaceAll("//|/|\\.", "\\" + File.separator)
        + realFileName;
