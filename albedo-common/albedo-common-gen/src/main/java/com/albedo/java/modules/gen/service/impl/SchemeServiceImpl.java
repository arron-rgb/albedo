...
public class SchemeServiceImpl extends DataServiceImpl<SchemeRepository, Scheme, SchemeDto, String>
  ...
  public String generateCode(SchemeDto schemeDto) {
    ...
    tableDto.setColumnList(
      tableColumnService.list(Wrappers.<TableColumn>query().eq(TableColumn.F_SQL_GENTABLEID, tableDto.getId())).stream()
    ...
    if (childTableTemplateList.size() > 0) {
      tableDto.setChildList(
        tableRepository.selectList(Wrappers.<Table>lambdaQuery().eq(Table::getParentTable, tableDto.getId())).stream()
  ...
  public IPage getSchemeVoPage(PageModel pm, SchemeQueryCriteria schemeQueryCriteria) {
    ...
    pm.addOrder(OrderItem.desc("a." + Scheme.F_SQL_CREATED_DATE));
